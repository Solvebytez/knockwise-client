...
export function ManuallyAssignTerritoryModal({
  ...
  const [detectedResidents, setDetectedResidents] = useState<
    DetectedResident[]
  >([]);
  ...
  const [maxDistanceFromStreet, setMaxDistanceFromStreet] =
    useState<number>(1.0);
  const [buildingTypeFilter, setBuildingTypeFilter] =
    useState<string>("residential");
  const [addressValidationLevel, setAddressValidationLevel] =
    useState<string>("strict");
  const [coordinatePrecision, setCoordinatePrecision] =
  ...
  const [municipalityInputValue, setMunicipalityInputValue] =
    useState<string>("");
  ...
  const [isSettingAreaProgrammatically, setIsSettingAreaProgrammatically] =
    useState(false);
  ...
  const [isLoadingCommunities, setIsLoadingCommunities] =
    useState<boolean>(false);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !municipalityDropdownRef.current.contains(target)
      ) {
          ...
          const areas = data
            .filter((item: any) => {
              ...
              const hasHighImportance =
                item.importance && item.importance > 0.3;
  ...
  const searchMunicipalities = useCallback(
    async (query: string) => {
      ...
      console.log(
        "🔍 Manual search for municipalities:",
        query,
        "in area:",
        selectedArea
      );
      ...
      try {
        ...
        if (selectedArea.toLowerCase().includes("toronto")) {
          ...
          for (const searchQuery of searchQueries) {
            try {
              ...
              console.log(
                `Municipality search query "${searchQuery}" failed:`,
                error
              );
              continue;
            }
        ...
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
            areaQuery
          )}&addressdetails=1&limit=10&countrycodes=ca`
        );
        const data = await response.json();
    ...
    async (query: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
            communityQuery
          )}&addressdetails=1&limit=10&featuretype=suburb`
        );
        const data = await response.json();
    ...
    async (query: string) => {
      ...
      try {
        ...
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
            mapArtQuery
          )}&addressdetails=1&limit=10&featuretype=neighbourhood`
        );
        const data = await response.json();
        ...
        const mapArts = data
          .filter(
            (item: any) =>
              item.type === "neighbourhood" || item.type === "suburb"
          )
          .map((item: any) => ({
  ...
  const loadMunicipalitiesForArea = useCallback(async (areaName: string) => {
    ...
    try {
      ...
      if (areaName.toLowerCase().includes("toronto")) {
        ...
        for (const community of torontoCommunities.slice(0, 20)) {
          // Limit to first 20 for performance
          try {
            const searchQuery = `${community}, Toronto, Ontario, Canada`;
            const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
              searchQuery
            )}&addressdetails=1&limit=3&countrycodes=ca`;
        ...
        console.log(
          "Auto-loaded Toronto municipalities:",
        ...
        try {
          ...
          const municipalities = data
            .filter((item: any) => {
              ...
              const validClasses = ["place", "boundary"];
              const hasHighImportance =
                item.importance && item.importance > 0.2;
      ...
      // Remove duplicates
      const uniqueMunicipalities = allMunicipalities.filter(
        (municipality, index, self) =>
          index === self.findIndex((m) => m.id === municipality.id)
      );
  ...
  const loadCommunitiesForMunicipality = useCallback(
    async (municipalityName: string, areaName: string) => {
      ...
      setIsLoadingCommunities(true);
      console.log(
        "🔄 Auto-loading communities for municipality:",
        municipalityName,
        "in area:",
        areaName
      );
      ...
      try {
        ...
        for (const searchQuery of searchQueries) {
          try {
            ...
            console.log(
              `Community search query "${searchQuery}" failed:`,
              error
            );
            continue;
          }
  ...
  const loadStreetsForCommunity = useCallback(
    async (
      ...
      console.log(
        "🔄 Overpass API: Auto-loading residential streets for community:",
        communityName,
        "in municipality:",
        municipalityName,
        "area:",
      ...
      try {
        ...
        if (overpassStreets.length > 0) {
          ...
          console.log(
            `✅ Overpass API found ${formattedStreets.length} residential streets`
          );
        ...
        if (autocompleteService) {
          ...
          try {
            ...
            const googleResults = await new Promise<any[]>(
              (resolve, reject) => {
            ...
            const googleStreets = googleResults
              .filter((prediction) =>
                prediction.description.toLowerCase().includes("toronto")
              )
              .slice(0, 20);
            ...
            if (googleStreets.length > 0) {
              ...
              setShowStreetSuggestions(true); // Show the suggestions dropdown
              console.log(
                `✅ Google Places fallback found ${googleStreets.length} streets`
              );
              return;
            }
          } catch (error) {
        ...
        setShowStreetSuggestions(true); // Show the suggestions dropdown
        console.log(
          `✅ Known streets fallback found ${knownDownsviewStreets.length} streets`
        );
      } catch (error) {
        console.error(
          "Error auto-loading residential streets at community level:",
          error
        );
        setStreetSuggestions([]);
      } finally {
  ...
  const getCommunityBoundingBox = useCallback(
    async (communityName: string): Promise<number[]> => {
      console.log(
        "🗺️ Fetching dynamic bounding box for community:",
        communityName
      );
      ...
      try {
        ...
        for (let i = 0; i < queries.length; i++) {
          try {
            ...
            if (response.ok) {
              ...
              if (data.elements && data.elements.length > 0) {
                ...
                if (allPoints.length > 0) {
                  ...
                  console.log(
                    `✅ Dynamic bounding box for ${communityName} (query ${
                      i + 1
                    }):`,
                    bbox
                  );
            ...
            console.log(
              `❌ Query ${i + 1} failed for ${communityName}:`,
              error
            );
            continue;
          }
        ...
        throw new Error(
          `Could not find bounding box for community: ${communityName}`
        );
      } catch (error) {
        console.error(
          `❌ Failed to fetch dynamic bounding box for ${communityName}:`,
          error
        );
  ...
  const getBuildingsAlongStreet = useCallback(
    async (
      streetName: string,
      bufferDistance: number = 100
    ): Promise<any[]> => {
      console.log(
        "🏠 Overpass API: Fetching residential buildings along street:",
        streetName,
        "with buffer:",
        bufferDistance,
        "m"
      ...
      try {
        communityBbox = await getCommunityBoundingBox(
          selectedCommunity || "Centennial Park"
        );
        console.log(
          "🗺️ Using dynamic bounding box for",
          selectedCommunity,
          ":",
          communityBbox
        );
        ...
        console.error(
          "❌ Failed to get dynamic bounding box, cannot proceed with building detection"
        );
        return [];
      }
      ...
      try {
        ...
        if (buildings.length > 0) {
          console.log(
            `✅ Found ${buildings.length} residential buildings along ${streetName}`
          );
          console.log(
            "📍 Sample building coordinates:",
            buildings
              .slice(0, 3)
              .map((b: any) => ({ lat: b.lat, lng: b.lng, address: b.street }))
          );
          setDetectionProgress(
            `Found ${buildings.length} buildings on ${streetName}`
          );
          return buildings;
        } else {
          console.log(`❌ No buildings found for street: ${streetName}`);
          setDetectionProgress(
            `No buildings found on ${streetName} - trying alternative methods...`
          );
          return [];
        }
      } catch (error) {
        ...
        setDetectionProgress(
          `OpenStreetMap API failed for ${streetName} - using fallback methods...`
        );
  ...
  const searchStreetsInCommunity = useCallback(
    async (
      query: string,
      community: string,
      municipality: string,
      ...
      setIsLoadingStreets(true);
      console.log(
        "🔍 Searching streets in community hierarchy:",
        query,
        "within Community:",
        community,
        "Municipality:",
      ...
      try {
        ...
        for (const searchQuery of searchQueries) {
          try {
            const url = `https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
              searchQuery
            )}&addressdetails=1&limit=15&countrycodes=ca`;
            ...
            const streets = data
              .filter((item: any) => {
                ...
                const hasHighImportance =
                  item.importance && item.importance > 0.05;
                const isResidential =
            ...
            console.log(
              `Community street search query "${searchQuery}" failed:`,
              error
            );
            continue;
          }
        ...
        const uniqueStreets = allStreets
          ...
          .sort((a, b) =>
            a.structured_formatting.main_text.localeCompare(
  ...
  const handleCommunitySelect = useCallback(
    (community: any) => {
      ...
      console.log(
        "🔄 Auto-loading streets for community:",
        community.name,
        "in municipality:",
        selectedMunicipality,
        "area:",
  ...
  const validateGeographicHierarchy = useCallback(() => {
    ...
    if (!selectedCommunity) {
      warnings.push(
        "Community/Neighbourhood is recommended for better accuracy"
      );
    }
    ...
    if (selectedArea && selectedMunicipality) {
      console.log(
        "✅ Geographic hierarchy validation passed - data selected through cascading API calls"
      );
  ...
  const validateGeographicContainment = useCallback(
    async (lat: number, lng: number, level: string) => {
          ...
          case "municipality":
            return addressComponents.some(
              (comp: any) =>
                ...
                comp.long_name
                  .toLowerCase()
                  .includes(selectedMunicipality.toLowerCase())
            );
          case "community":
            return addressComponents.some(
              (comp: any) =>
                ...
                comp.long_name
                  .toLowerCase()
                  .includes(selectedCommunity.toLowerCase())
            );
    ...
    async (query: string) => {
      ...
      try {
        if (selectedMunicipality) {
          ...
          const communityContext = selectedCommunity
            ? `${selectedCommunity}, `
            : "";

          console.log("Geographic context for street search:", {
            municipalityName,
            areaContext,
            communityContext,
          });
  ...
  const searchStreetsGoogle = useCallback(
    async (query: string) => {
      console.log("🔍 searchStreetsGoogle called with:", query);

      if (!autocompleteService || query.length < 2) {
        console.log(
          "Google search conditions not met - autocompleteService:",
          !!autocompleteService,
          "query length:",
          query.length
        );
      ...
      try {
        ...
        autocompleteService.getPlacePredictions(
          request,
          (predictions: PlaceSuggestion[], status: any) => {
            console.log(
              "Google search response - status:",
              status,
              "predictions:",
              predictions?.length
            );
            if (
              ...
              const cleanedPredictions = predictions
                .filter((prediction) => {
                  ...
                  return (
                    name && name.length > 0 && !name.includes("EiFSAWRIYXUg")
                  ); // Filter out encoded strings
                })
                .map((prediction) => {
              ...
              console.log(
                "Setting Google street suggestions:",
                cleanedPredictions
              );
            } else {
              setStreetSuggestions([]);
  ...
  const loadStreetsForNeighbourhood = useCallback(
    async (municipalityName: string) => {
      if (!selectedMunicipality) return;

      setIsLoadingStreets(true);
      try {
        console.log(
          "🔄 Auto-loading streets for municipality:",
          municipalityName
        );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      const isOutsideStreetDropdown = !target.closest(
        '[data-dropdown="street"]'
      );
  ...
  const handleStreetSelect = (street: PlaceSuggestion) => {
    ...
    toast.info(
      '🔄 Street selected. Building detection data has been reset. Please run "Detect Residents" again.'
    );
    setShowStreetSuggestions(false);
  ...
  const removeStreet = (placeId: string) => {
    ...
    // Inform user that data has been reset
    toast.info(
      '🔄 Street removed. Building detection data has been reset. Please run "Detect Residents" again.'
    );
  };
  ...
  const calculateTerritoryArea = useCallback((polygon: any): number => {
    ...
    try {
      ...
      console.log(
        `📊 Turf.js area calculation: ${area} sq meters = ${(
          area / 10000
        ).toFixed(2)} hectares`
      );
      return area;
  ...
  const isResidentialAddress = useCallback(
    (lat: number, lng: number, polygon: any): boolean => {
      if (!polygon || polygon.length < 3) {
      ...
      try {
        ...
        console.log(
          `📍 Turf.js geospatial check: (${lat}, ${lng}) inside polygon: ${isInside}`
        );
        return isInside;
      } catch (error) {
  ...
  const generateEnhancedPolygon = useCallback(
    (residents: DetectedResident[]): any => {
      ...
      try {
        ...
        console.log(
          `🗺️ Generated enhanced polygon with ${bufferedBbox.length} points`
        );
        return bufferedBbox;
      } catch (error) {
  ...
  const detectResidentsFromStreets = useCallback(async () => {
    ...
    if (hierarchyValidation.errors.length > 0) {
      toast.error(
        `Geographic validation failed: ${hierarchyValidation.errors.join(", ")}`
      );
      return;
    }
    ...
    if (hierarchyValidation.warnings.length > 0) {
      toast.warning(
        `Geographic warnings: ${hierarchyValidation.warnings.join(", ")}`
      );
    }
    ...
    setApiCallCount(0);
    toast.info(
      "🚀 Starting enhanced OSM-based resident detection with geographic hierarchy..."
    );
    ...
    try {
      ...
      console.log(
        "🔍 Starting enhanced detection for",
      ...
      for (const streetId of selectedStreets) {
        ...
        if (streetId.startsWith("overpass_")) {
          ...
          const overpassStreet = streetSuggestions.find(
            (street) => street.place_id === streetId
          );
          if (overpassStreet) {
            ...
            const buildings = await getBuildingsAlongStreet(
              streetName,
              searchRadius
            );
            if (buildings.length > 0) {
              console.log(
                `🏠 Found ${buildings.length} buildings along ${streetName}`
              );
              ...
              console.log(
                `✅ Added ${residents.length} residents from Overpass API for ${streetName}`
              );
            }
            continue; // Skip the rest of the processing for Overpass streets
          } else {
            console.error(
              "❌ Overpass street not found in cached data:",
              streetId
            );
            continue;
          }
        ...
        try {
          ...
          const bbox = osmService.createTightBoundingBox(
            streetInfo,
            searchRadius
          );
          ...
          const processedBuildings = osmService.processOSMBuildings(
            osmBuildings,
            streetName
          );

          // Apply geographic hierarchy filtering
          const geographicallyFilteredBuildings = processedBuildings.filter(
            (building) => {
          ...
          console.log(
            `🏢 OSM found ${processedBuildings.length} buildings, ${geographicallyFilteredBuildings.length} after geographic filtering for ${streetName}`
          );

          // Convert OSM buildings to DetectedResident format
          const streetResidents: DetectedResident[] =
            geographicallyFilteredBuildings.map((building, index) => ({
              id: `osm_${building.id}_${index}`,
              name: `Resident ${building.id}`,
              address: building.address || `${building.id} ${streetName}`,
              buildingNumber: building.buildingNumber
                ? parseInt(building.buildingNumber)
                : undefined,
              lat: building.lat,
              lng: building.lng,
          ...
          console.log(
            `✅ Street "${streetName}" processed: ${streetResidents.length} residents found`
          );
          toast.success(
            `Found ${streetResidents.length} residents on ${streetName}`
          );
        } catch (error) {
          console.error(
            `❌ OSM building detection failed for ${streetName}:`,
            error
          );
        }
      }
      ...
      console.log(
        `🎉 Enhanced detection completed: ${allResidents.length} residents from ${selectedStreets.length} streets`
      );
      console.log(
        `📊 Territory stats: Area: ${(area / 10000).toFixed(
          2
        )} ha, Density: ${density.toFixed(1)} buildings/ha`
      );
      console.log(
        `📊 Final results: ${allResidents.length} validated residents, ${
          enhancedPolygon?.length || 0
        } polygon points`
      );
      console.log(
        `📊 API Call Statistics: ${totalApiCalls} total API calls made`
      );
      console.log(`🗺️ Building-based polygon bounds:`, enhancedPolygon);

      // Enhanced success toast with detailed stats
      toast.success(
        `✅ Enhanced detection complete! Found ${
          allResidents.length
        } residents across ${selectedStreets.length} streets. Area: ${(
          area / 10000
        ).toFixed(2)} ha, Density: ${density.toFixed(1)} buildings/ha`
      );
    } catch (error) {
  ...
  const extractBuildingNumberFromComponents = (components: any[]): number => {
    ...
    const streetNumber = components.find(
      (comp) => comp.types && comp.types.includes("street_number")
    );
    ...
    const subpremise = components.find(
      (comp) => comp.types && comp.types.includes("subpremise")
    );
    ...
    const formattedAddress = components.map((comp) => comp.long_name).join(" ");
  ...
  const getOsmStreetInfo = async (streetId: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `https://nominatim.openstreetmap.org/lookup?osm_ids=W${wayId}&format=json&addressdetails=1`
      );
      const data = await response.json();
  ...
  const searchResidentialBuildings = async (
    streetInfo: any,
    streetName: string,
    territoryPolygon?: any
  ): Promise<DetectedResident[]> => {
    console.log(
      "🏠 Starting enhanced residential building search for:",
      streetName
    );
    ...
    try {
      const center = streetInfo.geometry?.location || {
        lat: streetInfo.lat,
        lng: streetInfo.lon,
      };
      const searchQueries = [
      ...
      for (const query of searchQueries) {
        try {
          ...
          buildings.forEach((building) => {
            ...
            const buildingNumber = extractBuildingNumber(
              building.name || building.formatted_address
            );
            ...
            if (
              ...
              console.log(
                `🏠 Found residential building: ${buildingNumber} at ${building.formatted_address}`
              );
            }
          });

          console.log(
            `✅ Query "${query}" found ${buildings.length} buildings`
          );
      ...
      if (residents.length === 0) {
        ...
        const houseNumbers = await searchForHouseNumbers(
          streetInfo,
          streetName
        );
      ...
      console.log(
        `🎯 Enhanced search completed: ${residents.length} residential buildings found`
      );
      return residents;
    } catch (error) {
  ...
  const reverseGeocodeResidentialBuildings = async (
    streetInfo: any,
    streetName: string,
    territoryPolygon?: any
  ): Promise<DetectedResident[]> => {
    ...
    try {
          ...
          results.forEach((result) => {
            ...
            if (
              ...
              const buildingNumber =
                extractBuildingNumberFromAddressValidation(result);
              ...
              if (
                ...
                console.log(
                  `🔄 Found via geocoding: ${buildingNumber} at ${address}`
                );
              }
            }
          });
      ...
      console.log(
        `🔄 Reverse geocoding completed: ${residents.length} buildings found`
      );
      return residents;
    } catch (error) {
  ...
  const deduplicateResidents = (
    residents: DetectedResident[]
  ): DetectedResident[] => {
    console.log("🔄 Deduplicating residents...");
    const uniqueResidents = residents.filter(
      (resident, index, self) =>
        index ===
        self.findIndex(
          (r) =>
    ...
    console.log(
      `🔄 Deduplication: ${residents.length} → ${uniqueResidents.length} residents`
    );
    return uniqueResidents;
  };

  // Sequential house number detection
  const detectSequentialHouseNumbers = async (
    residents: DetectedResident[],
    streetName: string
  ): Promise<DetectedResident[]> => {
    console.log(
      "🔢 Starting sequential house number detection for:",
      streetName
    );
    ...
    try {
      ...
      const missingNumbers = generateMissingHouseNumbers(
        residents,
        patterns,
        streetName
      );
      console.log(
        `🔢 Generated ${missingNumbers.length} missing house numbers`
      );
      ...
      console.log(
        `🔢 Sequential detection complete: ${allResidents.length} total residents`
      );
  ...
  const analyzeHouseNumberPatterns = (residents: DetectedResident[]) => {
    ...
    const mostCommonDifference = Object.entries(differenceCounts).sort(
      ([, a], [, b]) => b - a
    )[0][0];
  ...
  const generateMissingHouseNumbers = (
    residents: DetectedResident[],
    patterns: any,
    streetName: string
  ): number[] => {
    ...
    if (patterns.hasOddEven && patterns.step === 2) {
      console.log(
        "🔢 Detected odd/even pattern, generating numbers with step 2"
      );
      ...
      for (
        let i = patterns.min - extendRange;
        i < patterns.min;
        i += patterns.step
      ) {
        if (generated.length >= maxGenerated) break;
      ...
      for (
        let i = patterns.max + patterns.step;
        i <= patterns.max + extendRange;
        i += patterns.step
      ) {
        if (generated.length >= maxGenerated) break;
    ...
    console.log(
      "🔢 Generated missing house numbers:",
      generated.length,
      "numbers"
    );
    return generated;
  ...
  const estimateBuildingPosition = (
    number: number,
    residents: DetectedResident[],
    patterns: any,
    streetName: string
  ...
  const searchForHouseNumbers = async (
    streetInfo: any,
    streetName: string,
    territoryPolygon?: any
  ): Promise<DetectedResident[]> => {
    ...
    try {
      ...
      console.log(
        `🔍 House number search completed: ${residents.length} residents found`
      );
      return residents;
    } catch (error) {
  ...
  const validateTerritoryData = useCallback((data: any) => {
    ...
    if (
      ...
      data.boundary.coordinates.length === 0
    ) {
      ...
      if (!polygonCoords || polygonCoords.length < 3) {
        errors.push(
          "Territory boundary must have at least 3 coordinate points"
        );
      }
    }
    ...
    if (
      !data.buildingData ||
      !data.buildingData.addresses ||
      data.buildingData.addresses.length === 0
    ) {
    ...
    if (
      ...
      if (coords.length > 0) {
        ...
        if (first[0] !== last[0] || first[1] !== last[1]) {
          errors.push(
            "Territory boundary polygon must be closed (first and last points must be identical)"
          );
        }
      }
    }
  ...
  const checkZoneOverlap = useCallback(
    async (boundary: any, buildingData: any) => {
      try {
        ...
        if (response.data.success) {
          const { hasOverlap, overlappingZones, duplicateBuildings, isValid } =
            response.data.data;

          if (hasOverlap) {
            const zoneNames = overlappingZones
              .map((zone: any) => zone.name)
              .join(", ");
            toast.error(
              `❌ Territory overlaps with existing zones: ${zoneNames}`
            );
            return false;
          }
          ...
          if (duplicateBuildings && duplicateBuildings.length > 0) {
            toast.error(
              `❌ ${duplicateBuildings.length} buildings are already assigned to other territories`
            );
            return false;
          }
  ...
  const saveTerritory = useCallback(async () => {
    ...
    try {
      ...
      const polygonCoords = generatedPolygon.map((coord: any) => [
        coord.lng,
        coord.lat,
      ]);
      ...
      const buildingData = {
        ...
        coordinates: detectedResidents.map((resident) => [
          resident.lng,
          resident.lat,
        ]),
        totalBuildings: detectedResidents.length,
        residentialHomes: detectedResidents.filter((r) => r.buildingNumber)
          .length,
      };
      ...
      const territoryData = {
        ...
        ...(selectedAssignment &&
          assignmentType === "team" && { teamId: selectedAssignment._id }),
      ...
      const overlapCheckPassed = await checkZoneOverlap(
        territoryData.boundary,
        territoryData.buildingData
      );
      ...
      if (error && typeof error === "object" && "response" in error) {
        ...
        if (axiosError.response?.data?.message) {
          ...
          if (errorMessage.includes("already exists")) {
            toast.error(
              `❌ Territory name "${territoryName}" already exists. Please choose a different name.`
            );
          } else if (errorMessage.includes("overlaps")) {
            toast.error(
              `❌ Territory overlaps with existing zones. Please adjust the boundaries.`
            );
          } else if (errorMessage.includes("duplicate buildings")) {
            toast.error(
              `❌ Some buildings are already assigned to other territories.`
            );
          } else {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "❌ Unknown error occurred while saving territory"
      );
    } finally {
      setIsSavingTerritory(false);
    }
  ...
  const handleStep2Submit = async (e: React.FormEvent) => {
    ...
    if (selectedAssignment && assignedDate && savedTerritoryId) {
      ...
      try {
        ...
        if (assignmentResponse.data) {
          ...
          if (isScheduled) {
            ...
            toast.success(
              `Territory "${territoryName}" has been scheduled for assignment to ${assignedToName} on ${new Date(
                assignedDate
              ).toLocaleDateString()}`
            );
          } else {
            ...
            toast.success(
              `Territory "${territoryName}" has been assigned to ${assignedToName} (${assignmentType})`
            );
          }
        } else {
        ...
        toast.error(
          error instanceof Error
            ? error.message
            : "❌ Unknown error occurred while assigning territory"
        );
        return; // Don't close modal on error
      } finally {
        setIsAssigningTerritory(false);
      ...
      toast.info(
        "Territory saved without assignment. You can assign it later."
      );
    }
      ...
      <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto m-4">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          ...
          <div className="flex items-center justify-center space-x-4 mt-4">
            ...
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center border-2 font-semibold text-sm ${
                  formStep >= 1
          ...
          <form onSubmit={handleStep1Submit} className="px-6 py-4 space-y-6">
            ...
            <div className="space-y-4">
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        if (isSettingAreaProgrammatically) {
                          console.log(
                            "🔄 Programmatically setting area, skipping search"
                          );
                          return;
                        }
                        ...
                        if (selectedArea && e.target.value === selectedArea) {
                          console.log(
                            "🔄 Value matches selected area, not searching"
                          );
                          setAreaSuggestions([]);
                          return;
                        }
                    ...
                    <Input
                      ...
                      onFocus={() => {
                        ...
                        if (selectedArea && cachedMunicipalities.length > 0) {
                          console.log(
                            "🔄 Showing cached municipalities on focus:",
                            cachedMunicipalities.length
                          );
                          ...
                          console.log(
                            "🔄 No cache found, loading municipalities for area:",
                            selectedArea
                          );
                    ...
                    <Input
                      ...
                      onFocus={() => {
                        ...
                        if (
                          ...
                          console.log(
                            "🔄 Showing cached communities on focus:",
                            cachedCommunities.length
                          );
                          ...
                          console.log(
                            "🔄 No cache found, loading communities for municipality:",
                            selectedMunicipality
                          );
                          loadCommunitiesForMunicipality(
                            selectedMunicipality,
                            selectedArea
                          );
                        }
                      ...
                      placeholder={
                        selectedMunicipality
                          ? "Search for community/neighbourhood"
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (selectedCommunity && e.target.value.length >= 1) {
                        console.log(
                          "Searching streets within community hierarchy:",
                          selectedCommunity
                        );
                        searchStreetsInCommunity(
                          e.target.value,
                          selectedCommunity,
                          selectedMunicipality,
                          selectedArea
                        ...
                        console.log(
                          "Not calling street search - conditions not met"
                        );
                        setShowStreetSuggestions(false);
                        setShowResultsPanel(false);
                      }
                    ...
                    onFocus={() => {
                      ...
                      if (selectedCommunity && cachedStreets.length > 0) {
                        console.log(
                          "🔄 Showing cached streets on focus (Community level):",
                          cachedStreets.length
                        );
                        setStreetSuggestions(cachedStreets);
                        setShowStreetSuggestions(true);
                        setShowResultsPanel(false);
                    ...
                    <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-48 overflow-y-auto">
                      ...
                      {streetSuggestions.map((street) => (
                        ...
                        >
                          ...
                          <div className="text-sm text-gray-500">
                            {street.structured_formatting.secondary_text}
                          </div>
                        </button>
                      ))}
                    </div>
                ...
                {selectedMunicipality && !isLoadingStreets && (
                  <div className="flex justify-end mt-2">
                    <Button
                      ...
                      onClick={() => {
                        console.log(
                          "🔄 Loading streets manually for municipality:",
                          selectedMunicipality
                        );
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                  ...
                  >
                    ...
                    <div className="truncate">
                      {selectedMunicipality || "Not selected"}
                    </div>
                  </div>
                  <div
                    className={`p-2 rounded ${
                  ...
                  >
                    ...
                    <div className="truncate">
                      {selectedCommunity || "Not selected"}
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedStreets.map((streetId) => {
                      const street = streetSuggestions.find(
                        (s) => s.place_id === streetId
                      );
                      const streetName =
                        street?.structured_formatting?.main_text ||
                        street?.description ||
                    ...
                    detectedResidents.length === 0 && (
                      <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                        ...
                        <p className="text-xs text-amber-700 mt-1">
                          Streets have been selected. Please run "Detect
                          Residents" to generate building data and territory
                          polygon.
                        </p>
                      </div>
                    )}
              ...
              <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div className="space-y-2">
                    <Label
                      htmlFor="addressValidationLevel"
                      className="text-xs font-medium text-blue-700"
                    >
                      Address Validation Level
                    </Label>
                    <Select
                      value={addressValidationLevel}
                      onValueChange={setAddressValidationLevel}
                    >
                      <SelectTrigger className="h-8 text-xs border-blue-300 focus:border-blue-500 focus:ring-blue-500/20">
                        <SelectValue />
                      </SelectTrigger>
                ...
                <div className="mt-3 p-2 bg-blue-100 rounded border border-blue-300">
                  ...
                  <div className="text-xs text-blue-700">
                    🔍 Search Radius: {searchRadius}m | 📏 Max Distance:{" "}
                    {maxDistanceFromStreet}km | 🏢 Type: {buildingTypeFilter} |
                    ✅ Validation: {addressValidationLevel} | 📍 Precision:{" "}
                    {coordinatePrecision}
                  </div>
                </div>
              </div>
                            ...
                            <div className="flex-1">
                              ...
                              <div className="text-xs text-gray-500 space-y-1">
                                <div>
                                  ...
                                  <span className="ml-1">
                                    {resident.lat.toFixed(6)},{" "}
                                    {resident.lng.toFixed(6)}
                                  </span>
                                </div>
                                <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            ...
                            <span className="ml-1 font-medium">
                              {territoryArea > 0
                                ? `${territoryArea.toFixed(2)} ha (${Math.round(
                                    territoryArea * 10000
                                  )} sq meters)`
                                : "Calculating..."}
                            </span>
            ...
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <div className="text-sm text-blue-800">
                <strong>Step 1:</strong> Enter territory information and select
                the geographical hierarchy. Start with Area/Region, then
                Municipality/City, then Community/Neighbourhood. Finally, search
                for streets within the selected area. Residents will be
                automatically detected from the selected streets.
              </div>
            </div>
        ...
        {formStep === 2 && (
          <form onSubmit={handleStep2Submit} className="px-6 py-4 space-y-6">
                                ...
                                <div className="mt-1">
                                  ...
                                  {result.assignmentSummary
                                    ...
                                    <div className="mt-1 space-y-1">
                                      {result.assignmentSummary
                                        .assignmentDetails.isFullyAssigned && (
                                      ...
                                      {result.assignmentSummary
                                        .assignmentDetails.isOnlyScheduled && (
                                      ...
                                      <div className="text-xs text-gray-600">
                                        ...
                                        {result.assignmentSummary
                                          ...
                                          <span className="inline-block mr-2">
                                            👥{" "}
                                            {
                                              result.assignmentSummary.teamZones
                                                .length
                                            }{" "}
                                            team
                                          </span>
                                ...
                                result.teamMemberships.length > 0 && (
                                  <div className="mt-1">
                                    ...
                                    <p className="text-xs text-amber-600 mt-1">
                                      Already in {result.teamMemberships.length}{" "}
                                      team(s)
                                    </p>
                                  </div>
                                )}
                ...
                {assignmentSearchQuery.trim() &&
                  ...
                  assignmentSearchResults.length === 0 && (
                    <div className="mt-2 p-3 text-center text-sm text-gray-500 bg-gray-50 border border-gray-200 rounded-md">
                      No {assignmentType === "team" ? "teams" : "agents"} found
                      matching "{assignmentSearchQuery}"
                    </div>
                  )}
            ...
            <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
              <div className="text-sm text-blue-800">
                <strong>Step 2:</strong> Optionally assign the territory to a
                sales representative. You can skip this step and assign later.
              </div>
            </div>
