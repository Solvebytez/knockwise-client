...
export function TerritoryEditMap({ 
  ...
  const [internalShowExistingTerritories, setInternalShowExistingTerritories] = useState(true)
  const showExistingTerritories = externalShowExistingTerritories !== undefined ? externalShowExistingTerritories : internalShowExistingTerritories
  const setShowExistingTerritories = externalOnToggleExistingTerritories || setInternalShowExistingTerritories
  
  // Drawing manager state
  const drawingManagerRef = useRef<any | null>(null)
  const [drawingManagerInstance, setDrawingManagerInstance] = useState<any>(null)
  ...
  useEffect(() => {
    const fetchExistingTerritories = async () => {
      try {
        ...
        console.log('Fetched existing territories:', territories)
        console.log('Current territory ID:', territory?._id)
        console.log('Territories with boundaries:', territories.filter((t: any) => t.boundary?.coordinates))
  ...
  useEffect(() => {
    if (mapRef && territory?.boundary?.coordinates) {
      console.log('Force focusing on territory after data change:', territory.name)
      const bounds = new google.maps.LatLngBounds()
      territory.boundary.coordinates[0].forEach((coord: number[]) => {
        bounds.extend({ lat: coord[1], lng: coord[0] })
      })
  ...
  useEffect(() => {
    if (isEditingBoundary) {
      ...
      if (drawingManagerInstance) {
        drawingManagerInstance.setDrawingMode(google.maps.drawing.OverlayType.POLYGON)
      }
    } else {
    ...
    <div className="flex h-full">
      <div className="flex-1 relative">
        <GoogleMap
          ...
          center={territory?.boundary?.coordinates ? undefined : { lat: 43.6532, lng: -79.3832 }}
        ...
        >
          ...
          {territory?.boundary?.coordinates && (
            <Polygon
              paths={territory.boundary.coordinates[0].map((coord: number[]) => ({
                lat: coord[1],
                lng: coord[0]
              }))}
              options={{
          ...
          {territory?.boundary?.coordinates && (
            <Marker
              position={{
                lat: territory.boundary.coordinates[0].reduce((sum: number, coord: number[]) => sum + coord[1], 0) / territory.boundary.coordinates[0].length,
                lng: territory.boundary.coordinates[0].reduce((sum: number, coord: number[]) => sum + coord[0], 0) / territory.boundary.coordinates[0].length,
              }}
              icon={{
          ...
          {showExistingTerritories && existingTerritories.map((existingTerritory) => {
            // Skip the current territory being edited
        ...
        {isLoadingTerritories && (
          <div className="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center z-30">
            <div className="bg-white rounded-lg p-6 shadow-lg flex items-center gap-3">
              ...
              <span className="text-gray-700 font-medium">Loading existing territories...</span>
            </div>
          </div>
        )}
            ...
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 z-20">
              <div className="bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg flex items-center gap-2">
                ...
                <span className="font-medium">
                  {drawnBoundary ? 'Boundary Drawn - Click "Update Territory" to Apply' : 'Drawing Mode Active - Draw your new boundary'}
                </span>
                                                   <Button
                     variant="ghost"
                     size="sm"
