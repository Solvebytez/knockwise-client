...
export function TerritoryMap() {
  ...
  const handleSaveTerritory = () => {
    ...
    // Open community assignment modal instead of saving immediately
  ...
  const handleAssignTerritory = async () => {
    console.log("handleAssignTerritory called with:", {
      selectedTerritory,
      selectedAssignment,
      assignedDate,
      workflowStep,
    ...
    if (
      (selectedTerritory || currentZoneIdRef.current) &&
      selectedAssignment &&
      assignedDate
    ) {
      ...
      try {
        console.log("Assigning territory:", {
          territory: selectedTerritory?.name || "Unknown",
          assignment: selectedAssignment?.name || "Unknown",
          type: assignmentType,
          date: assignedDate,
        });

        console.log("Selected territory object:", selectedTerritory);
        console.log(
          "Zone ID being used:",
          selectedTerritory?.id || currentZoneIdRef.current
        );

        // Prepare assignment data - use ref as fallback if selectedTerritory is not available
        const zoneId = selectedTerritory?.id || currentZoneIdRef.current;
        console.log("Using zoneId for assignment:", zoneId);
        ...
        console.log("zoneId value:", zoneId);
        ...
        if (assignmentResponse.data) {
          ...
          if (isScheduled) {
            ...
            toast.success(
              `"${
                selectedTerritory?.name || "Territory"
              }" has been assigned to ${assignedToName} (${assignmentType})`
            );
          }
        ...
        if (error && typeof error === "object" && "response" in error) {
          ...
          // Extract error message with robust handling
          let errorMessage = "Failed to assign territory. Please try again.";
          ...
          if (axiosError.response?.data) {
            ...
            if (axiosError.response.status === 422) {
              if (errorMessage.toLowerCase().includes("zone id")) {
                errorMessage =
                  "Invalid zone data. Please try creating the zone again.";
              ...
              if (!errorMessage.includes("already assigned")) {
                errorMessage =
                  "This territory is already assigned. Please choose a different territory.";
              }
            } else if (axiosError.response.status === 404) {
              errorMessage =
                "Territory or assignment not found. Please refresh and try again.";
            }
          }

          console.log("Displaying assignment error toast:", errorMessage);
  ...
  // Community assignment modal handlers
  const handleCommunityModalClose = () => {
    setShowCommunityModal(false);
    setCurrentZoneId("");
    setCurrentZoneName("");
    // Keep the current workflow step - don't reset to drawing mode
    // The user should be able to try again without losing their drawn zone
  };

  const handleCommunityAssignmentSuccess = (
    zoneId: string,
    communityId: string,
    communityName: string
  ) => {
    console.log("=== COMMUNITY ASSIGNMENT SUCCESS ===");
    ...
    console.log("zoneId type:", typeof zoneId);
    console.log("zoneId length:", zoneId?.length);

    // Validate zoneId
    if (!zoneId || typeof zoneId !== "string" || zoneId.trim() === "") {
      console.error("❌ Invalid zoneId received:", zoneId);
      toast.error("Invalid zone ID received. Please try again.");
      return;
    }

    // Store the zone ID in the ref for immediate access
    currentZoneIdRef.current = zoneId;

    // Close the modal
    setShowCommunityModal(false);

    // Create the territory object for local state
    const newTerritory = {
      id: zoneId,
    ...
    console.log("Created new territory object:", newTerritory);

    // Add to local store and select it
    addTerritory(newTerritory);
    selectTerritory(newTerritory);

    console.log("Territory added and selected:", newTerritory);
    console.log(
      "Current selectedTerritory after selection:",
      selectedTerritory
    );
    console.log(
      "currentZoneIdRef.current after setting:",
    ...
    // Proceed to team/individual assignment step
  ...
  return (
    <div className="flex h-full mt-4">
      ...
      <ZoneCommunityAssignmentModal
        ...
        onSuccess={handleCommunityAssignmentSuccess}
        mode="create"
        zoneData={{
          name: territoryName.trim(),
          description: territoryDescription.trim(),
          polygon: pendingTerritory?.polygon,
          buildingData: pendingTerritory
            ? {
                addresses: pendingTerritory.residentsData.map(
                  (resident: any) => resident.address
                ),
                coordinates: pendingTerritory.residentsData.map(
                  (resident: any) => [resident.lng, resident.lat]
                ),
              }
            : undefined,
        }}
